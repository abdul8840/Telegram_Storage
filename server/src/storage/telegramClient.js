...
export async function getAccount(userId) {
  return db.tgAccounts.findOne({ userId });
}
...
async function upsertAccount(userId, patch) {
  ...
  if (existing) {
    await db.tgAccounts.updateOne({ _id: existing._id }, { $set: { ...patch, updatedAt: now } });
    return { ...existing, ...patch, updatedAt: now };
  }
  const doc = { _id: randomId(10), userId, createdAt: now, updatedAt: now, status: 'pending', ...patch };
...
export async function getClient(userId) {
  ...
  if (cached) {
    touch(cached);
    if (cached.connected && cached.client.connected) return cached;
    clients.delete(key);
  ...
  if (account.status !== 'active' || !account.sessionString) {
    throw ApiError.badRequest('Your Telegram connection is incomplete. Please finish the login flow.', null, 'TG_NOT_CONNECTED');
  }
  ...
  try {
    ...
    if (!authorized) {
      ...
      await db.tgAccounts.updateOne(
        { userId },
        { $set: { status: 'unauthorized', lastError: 'Session is no longer authorised by Telegram', updatedAt: new Date().toISOString() } },
      );
      throw ApiError.unauthorized('Your Telegram session is no longer valid. Please reconnect your Telegram account.');
    }
    entry.connected = true;
    touch(entry);
    log.info(`connected to Telegram as ${account.firstName || account.phone || userId}`);
    return entry;
  } catch (err) {
    clients.delete(key);
    ...
    await db.tgAccounts
      .updateOne({ userId }, { $set: { lastError: message, updatedAt: new Date().toISOString() } })
      .catch(() => {});
